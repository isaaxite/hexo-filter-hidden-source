...
function addSourcename(item) {
  ...
  writeFileSync(CACNE_HIDDEN_SOURCE_PATH, JSON.stringify(cache));
}

/**
 * check source name if exist
 * @param {string} item - sourcename
 * @returns {boolean}
 */
function sourcenameIfExist(item) {
  const sourcenames = getSourcenames();
  return sourcenames.includes(item);
}
